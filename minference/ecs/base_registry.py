...
class BaseRegistry(Generic[T]):
    ...
    @classmethod
    def get(cls, key: Union[str, UUID]) -> Optional[T]:
        """Get an item from the registry by key."""
        cls._logger.debug(f"{cls.__name__}: Getting item with key {key}")
        return cls._registry.get(key)

    @classmethod
    def get_many(cls, keys: List[Union[str, UUID]]) -> List[Optional[T]]:
        """Get multiple items from the registry by their keys."""
        cls._logger.debug(f"{cls.__name__}: Getting {len(keys)} items")
        return [cls.get(key) for key in keys]

    @classmethod
    def set(cls, key: Union[str, UUID], value: T) -> None:
        """Set an item in the registry."""
        cls._logger.debug(f"{cls.__name__}: Setting item with key {key}")
        cls._registry[key] = value
        cls._record_timestamp(key)
